import { petState, state, type Platform } from '../state.svelte';
import { agentManager } from '../agent/agent_manager';
import { particleSystem } from '../particleSystem';

export function executeLegacyBehaviors(bounds: any, currentGroundY: number, targetPlatform: Platform | null) {
  const { leftWall, rightWall, ceilingY, groundY, centerX, centerY, W, H } = bounds;
  
  const triggerClimbFall = (msg: string, vx: number, vy: number) => {
    petState.currentAction = 'falling';
    petState.vx = vx;
    petState.vy = vy;
    petState.actionTimer = 0;
    (petState as any).climbOutcome = undefined;
...
if (state.isDragging) {
  ...
  if (petState.currentAction === 'flipped') {
    ...
    if (petState.mood === 'crying') {
      ...
      if (petState.actionTimer <= 80) {
        petState.mood = 'angry';
        state.showSpeech("Á à cái đồ bạo lực này! Cho tui lật lại mau! 😡", 3000);
      }
    }
  ...
  if (isOffScreen && petState.currentAction !== 'portal') {
    ...
    state.showSpeech("\u00da \u00f2a! Portal n\u00e8! \ud83c\udf00\ud83d\udc22", 3000);
    return;
  }
  ...
  if (petState.currentAction === 'climbing_ground_left') {
    ...
    if (petState.globalX <= leftWall) {
      ...
      if ((petState as any).climbCycles === 1) {
        ...
        state.showSpeech(successSpeeches[Math.floor(Math.random() * successSpeeches.length)], 3500);
      } else {
  ...
  if (petState.currentAction === 'climbing_ceiling') {
    ...
    if ((petState as any).climbOutcome === 'stuck' && petState.globalX >= fallThresholdX) {
      ...
      petState.actionTimer = 0;
      petState.mood = 'scared';
      state.showSpeech('Cứu tui! Kẹt trên trần nhà rồi, không dám buông tay... 🥺🕸️', 4000);
      return;
    }
      ...
      if (Math.abs(petState.vy) > 3.5) {
        ...
        if ((petState as any).isSelfFlipping) {
          ...
          if (petState.mood === 'crying') {
            ...
            state.showSpeech("Huhu lật mai rồi đau quá cứu tui với! 😭💦", 3000);
          } else {
            petState.mood = 'angry'; 
          ...
          if (petState.mood === 'crying') {
            ...
            state.showSpeech(crySpeeches[Math.floor(Math.random() * crySpeeches.length)], 3000);
            ...
            petState.squash = 0.6; 
            petState.stretch = 1.2;
            state.showSpeech("Phù... rớt từ trên cao mệt quá! 🥵💦", 3000);
          } else {
            petState.currentAction = 'wander';
            petState.squash = 0.6; 
            petState.stretch = 1.2;
  ...
  if (petState.currentAction === 'crying_ground') {
    ...
    if (petState.actionTimer > 150) { // ~2.5 seconds
      ...
      petState.mood = 'tired';
      petState.actionTimer = 0;
      state.showSpeech("Phù... đỡ đau hơn rồi! 🥵💦", 2500);
    }
    return;
  }
  ...
  if (petState.currentAction === 'bathing') {
    ...
    if (petState.actionTimer > 180) { // ~3 seconds
      ...
      petState.currentAction = 'idle';
      petState.actionTimer = 0;
      state.showSpeech("Ăn xong rồi ngon quá, đầy năng lượng luôn! 😋🐢", 3000);
    }
    return;
  }
    ...
    if (petState.actionTimer > 180) { // ~3 seconds
      ...
      petState.currentAction = 'idle';
      petState.actionTimer = 0;
      state.showSpeech("Phù, uống nước boba đã ghê! 💦🐢", 3000);
    }
    return;
  }
    ...
    if (petState.actionTimer > 100) { // ~1.6 seconds
      ...
      if (petState.hatType === 'none' || !petState.hatType) {
        ...
        state.showSpeech("Đội nón dev ngầu chưa chủ nhân? 😎🧢", 3000);
      } else if (petState.hatType === 'coder') {
        petState.hatType = 'wizard';
        state.showSpeech("Rùa phù thủy phép thuật hô biến! 🧙‍♂️✨", 3000);
      } else if (petState.hatType === 'wizard') {
        petState.hatType = 'straw';
        state.showSpeech("Kaizoku ou ni ore wa naru! Nón Luffy rơm! 👒🐢", 3000);
      } else {
        petState.hatType = 'none';
        state.showSpeech("Cởi nón ra cho nhẹ mát nào! 🐢", 2500);
      }
      petState.currentAction = 'idle';
      petState.actionTimer = 0;
    }
    ...
    if (petState.actionTimer > 240) { // ~4 seconds
      ...
      petState.currentAction = 'idle';
      petState.actionTimer = 0;
      state.showSpeech("Kiến thức dạt dào! Bug chỉ là chuyện nhỏ! 🧠🐢", 3000);
    }
    return;
  }
  ...
  if (petState.currentAction === 'hyper' && petState.actionTimer > 100 + Math.random() * 60) {
    ...
    state.showSpeech(brakeSpeeches[Math.floor(Math.random() * brakeSpeeches.length)], 3000);
  }
  ...
  if (petState.isFastClimb && (petState.currentAction === 'climbing_left' || petState.currentAction === 'climbing_right')) {
    if (petState.actionTimer > 85 + Math.random() * 40) {
      ...
      if (outcome && outcome !== 'success') {
        ...
        petState.isFastClimb = false;
        petState.actionTimer = 0;
        state.showSpeech("Áaaaa... trơn quá rớt rồi! 😭💥", 3000);
        return;
      } else {
  ...
  if (isWallOrCeilingClimb && !petState.isFastClimb && Math.random() < 0.02) {
    ...
    state.showSpeech(boostSpeeches[Math.floor(Math.random() * boostSpeeches.length)], 2500);
  }
